...
Different model pages, etc. Output to XML

https://taming-the-beast.org/workshops/Taming-the-BEAST-Squamish/
...
Runs MCMC, screenshot of PC version, examples of using cluster version via conda

SRUN Command: ```srun -p NMLResearch -c 4 --mem=16G beast -threads 4 Primates_long.xml```
SBATCH Command: ```sbatch -p NMLResearch -c 4 --mem=16G --wrap="beast -threads 4 Primates_long.xml"```
...
???
So that's a crash course in BEAST2 a *modular*, *extensible*, cross-platform package for performing Bayesian inference using MCMC with emphasis on phylogenetic analysis of molecular sequences which in keeping with the original definitions is also 
